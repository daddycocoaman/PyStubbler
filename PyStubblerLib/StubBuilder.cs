...
namespace PyStubblerLib
{
    public static class StubBuilder
    {
        ...
        public static string BuildAssemblyStubs(string targetAssemblyPath, BuildConfig config, string? destPath = null, string[]? searchPaths = null)
        {
            ...
            if (config.DestPathIsRoot && Directory.Exists(destPath))
            {
                stubsDirectory = new DirectoryInfo(destPath);
            }
            else
            {
                var extendedRootNS = config.Prefix + rootNamespace + config.Postfix;
